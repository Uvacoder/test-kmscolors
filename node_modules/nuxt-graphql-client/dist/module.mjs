...
function prepareTemplate(ctx) {
  ...
  ctx.clientTypes = Object.entries(ctx.template || {}).reduce((acc, [key, template]) => {
    acc[key] = template.match(/^export\stype\s\w+(?=\s=\s)/gm)?.filter((e) => !["Scalars", "SdkFunctionWrapper", "Sdk"].some((f) => e.includes(f))).map((e) => e.replace("export type ", ""));
    return acc;
  }, {});
}
...
const module = defineNuxtModule({
  ...
  async setup(opts, nuxt) {
    ...
    async function generateGqlTypes() {
      ...
      if (ctx.clientDocs) {
        ctx.template = ctx?.codegen ? await generate({
